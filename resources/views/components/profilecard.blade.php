<div class="bg-white shadow-lg rounded-lg">
    <div class="relative h-52">
        <div class="absolute bottom-2 left-2 z-20 w-full">
            <div class="text-white text-md lg:text-lg xl:text-xl 2xl:text-2xl font-medium" title="{{ $employee->fullname }}">{{ $employee->fullname }}</div>
    ...
    <div class="px-4 py-2">
        <div class="text-gray-500 text-lg lg:text-xl xl:text-2xl truncate" title="{{ $department." - ".$function }}">{{ $department." - ".$function }}</div>
        <div class="text-sm md:text-md xl:text-lg truncate">
            <a class="ml-2 text-red-700" title="{{ $employee->user->email }}">{{ $employee->user->email }}</a><br>
            <a class="ml-2 text-red-700" title="{{ $employee->phoneNumber }}">{{ $employee->phoneNumber }}</a>
